...
public class GemRepository
{
    ...
    /// The database
    /// </summary>
    private readonly GemVarContext _context;
    ...
    public Item? GetSvByVID(int vid)
    {
        ...
        {
            return ConvertSecsItemByDataType(Variable);
        }
        else
        {
            var subItem = _context.Variables.Where(v=>v.VarType=="SV").Where(v => v.ListSVID == vid).ToList();
            return Item.L(subItem.Select(v=> {
                if (v.DataType == "LIST")
                    return GetSvByVID(v.VID);
                return ConvertSecsItemByDataType(v);

                }).ToArray());
        }

    }
    Item ConvertSecsItemByDataType(GemVariable variable)
    {
        switch (variable.DataType)
        {
            ...
            case "UINT_1":
                ...
                return Item.U1( BitConverter.GetBytes(UINT_1) );
            case "UINT_2":
